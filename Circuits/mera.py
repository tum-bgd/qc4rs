'''
MERA BASED ON (BASARIM 2020? edit?)// GRANT HIERARCHICAL QUANTUM CLASSIFIERS // Tüsüz et al. Performance of Particle Tracking Using a Quantum Graph Network
'''
import cirq
import sympy
...
def create_mera(observable, grid):
    ...
    builder.add_ry(circuit, 3, 'ry' + str(n), 1)
    builder.add_ry(circuit, 4, 'ry' + str(n), 2)
    builder.add_cnot(circuit, data_qubits[5], data_qubits[6])
    builder.add_ry(circuit, 7, 'ry' + str(n), 3)
    builder.add_ry(circuit, 8, 'ry' + str(n), 4)
    builder.add_cnot(circuit, data_qubits[10], data_qubits[9])
    builder.add_ry(circuit, 11, 'ry' + str(n), 5)
    builder.add_ry(circuit, 12, 'ry' + str(n), 6)
