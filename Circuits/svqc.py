'''
SVQC BASED ON 'CIRCUIT-CENTRIC QUANTUM CLASSIFIERS' BY M. SCHULD
'''
import cirq
import sympy
...
class CircuitLayerBuilder_svqc():
    ...
    def add_twoqt_layer(self, circuit, qubits, rangeparam, gate, prefix):   
        controls=[]
        targets=[]
        r=rangeparam # MUST BE 1 FOR FIRST BLOCK AND 3 FOR SECOND BLOCK
...
def create_svqc(observable, grid):
    ...
    # builder.add_singleqt_layer(circuit, data_qubits, cirq.Rx, "ry" + str(k))
    ...
    # symbol = sympy.Symbol('ry2' + '-' + str(0))
    # ry = cirq.Rx(rads=symbol)
    # circuit.append(ry(readout))
